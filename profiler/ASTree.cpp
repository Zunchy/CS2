...
void AST::mainHeader(const std::vector<std::string>& profileName) {
	...
	std::string profHeader("#include \"profile.hpp\"\n");

	for (std::vector<std::string>::const_iterator it = profileName.begin(); it < profileName.end(); ++it){
	   	profHeader += "profile " + *it + "(\"" + *it  + "\")";
		profHeader.erase(profHeader.end()-6);
		profHeader.insert(profHeader.end()-5,'.'); 
		profHeader += ";\n";
	}

	profHeader += "\n";

	child.insert(index, new AST(token, profHeader));
...
void AST::fileHeader(const std::string& profileName) {
    ...
    profHeader = profHeader + "extern profile " + profileName + ";\n";
	...
	if((*it)->tag == "function" && (*it)->getChild("name")->getName() == "main") {
		...
		++blockIt;
	     ...
	     AST *output = new AST(token);
	     	...
	     	report += "std::cout << " + *i + " << std::endl; \n";


	     output->text = report;

             block->child.insert(blockIt, output);
...
void AST::lineCount(const std::string& profileName) {
    ...
    std::string expCount = "  " + profileName +".count(__LINE__); \n";

    while(it != child.end()) {

    	if(!(*it)->child.empty() && !isStopTag((*it)->tag))
      		(*it)->lineCount(profileName);


	 if((*it)->tag == "expr_stmt" && !isStopTag((*it)->tag)){
	...
	/******************************************** Attempt at the Challenge
		...
		if((*it)->tag == "condition"){
			...
			AST *count = new AST(token);
			count->text = expCount;
			(*it)->getChild("incr")->child.push_back(count);
